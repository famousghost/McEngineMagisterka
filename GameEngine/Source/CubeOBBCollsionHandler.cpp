...
std::vector<Meshes::LineSegment> CubeOBBCollsionHandler::getEdges(const Meshes::Collider& p_collider)
{
    ...
    for (int i = 0; i < 12; ++i) 
    {
        l_result.push_back(Meshes::LineSegment(glm::vec3(l_points[l_index[i][0]]), 
                                               glm::vec3(l_points[l_index[i][1]])));
    }
                                                            ...
                                                            const Meshes::Object& p_object)
{
    ...
    const auto& l_normals = p_collider.m_normals;
                                                   ...
                                                   bool& p_shouldFlip)
{
    ...
    if (!((l_intervalB.m_min <= l_intervalA.m_max) 
        and (l_intervalA.m_min <= l_intervalB.m_max))) 
    {
        return 0.0f;
    }
